"""Django Admin registration for bijay_dev models.
...
Features:
    ...
    ReadingList    — simple list, date hierarchy
    Thought        — simple list
    Book           — progress bar preview
"""
...
class ReadingListAdmin(admin.ModelAdmin):
    """Admin for bookmarked articles and links."""

    list_display = ("heading", "url", "added_date")
...
class ThoughtAdmin(admin.ModelAdmin):
    """Admin for short personal thoughts."""
...
class BookAdmin(admin.ModelAdmin):
    """Admin for books with progress tracking."""

    list_display = ("title", "author", "progress_bar", "date_started")
    search_fields = ("title", "author")
    readonly_fields = ("id", "created_at", "updated_at")
    ordering = ("-created_at",)

    @admin.display(description=_("Progress"))
    def progress_bar(self, obj: Book) -> str:
        """Render an inline progress bar showing reading progress."""
        pct = min(obj.percent_read, 100)
        color = "#16a34a" if pct == 100 else "#3b82f6" if pct >= 50 else "#f59e0b"
        return format_html(
            '<div style="width:100px;background:#e5e7eb;border-radius:4px;'
            'overflow:hidden;height:14px;" title="{}%">'
